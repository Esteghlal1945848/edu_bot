...
def major_keyboard(grade=None, state=None):
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    g = grade or state or ""
    kb.add(KeyboardButton(f"رشته:{g}|ریاضی"))
    kb.add(KeyboardButton(f"رشته:{g}|تجربی"))
    kb.add(KeyboardButton(f"رشته:{g}|انسانی"))
    kb.add(KeyboardButton("🔙 برگشت به منو"))
    return kb
